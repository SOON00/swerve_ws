...
#             if name in self.steer_joints:
#                 idx = self.steer_joints.index(name)
#                 steer_angles[idx] = msg.position[i]
#             elif name in self.wheel_joints:
#                 idx = self.wheel_joints.index(name)
#                 wheel_speeds[idx] = msg.velocity[i]
...
#         self.vx = sum(self.vx_modules) / 4.0
#         self.vy = sum(self.vy_modules) / 4.0

#         num = sum(self.lx[i] * self.vy_modules[i] - self.ly[i] * self.vx_modules[i] for i in range(4))
#         den = sum(self.lx[i]**2 + self.ly[i]**2 for i in range(4))
#         self.omega = num / den if den != 0 else 0.0

#     # ===== Timer → 적분 + odom publish =====
