...
def load_cbf_map():
    ...
    yaml_path = Path(pkg_share) / "params" / "swerve_map_params.yaml"
...
class BodyFrameCBF(Node):
    def __init__(self):
        super().__init__('cbf_filter_swerve_switching')
        ...
        self.lines, self.Pc2, self.Pc3, self.turn = parse_cbf_yaml(load_cbf_map())

        self.S = np.array([2.0, 0.0, 0.0])
        self.G = np.array([3.5, -1.5, -np.pi/2])
    ...
    def loop(self):
        ...
        if not self.cbf_enabled:
            pos_err_s = np.linalg.norm(self.x[0:2] - self.S[0:2])
            ang_err_s = abs(wrap_to_pi(self.x[2] - self.S[2]))

            if pos_err_s < self.start_tol_pos and ang_err_s < self.start_tol_ang:
                self.cbf_enabled = True
                self.get_logger().info("CBF Entered")
        ...
        # ========= Exit =========
        pos_err_g = np.linalg.norm(self.x[0:2] - self.G[0:2])
        ang_err_g = abs(wrap_to_pi(self.x[2] - self.G[2]))
        ...
        if pos_err_g < self.goal_tol_pos and ang_err_g < self.goal_tol_ang:
            ...
            self.publish_bool(self.cbf_active_pub, False)
            self.publish_bool(self.cbf_enable_pub, False)
            self.get_logger().info("CBF Exited")
            return
        ...
        u_nom_w = -np.array([
            ...
            self.p3 * wrap_to_pi(self.x[2] - self.G[2])
        ])
