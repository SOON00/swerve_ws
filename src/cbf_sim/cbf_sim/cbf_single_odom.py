...
def load_cbf_segments():
    ...
    yaml_path = Path(pkg_share) / "params" / "single_corner.yaml"
...
class MultiGoalCBF(Node):
    def __init__(self):
        super().__init__('cbf_single_odom')
        ...
        self.dl = 0.1
        self.db = 0.37
        self.v_max, self.w_max = 0.6, 2.5
        self.p1 = self.p2 = self.p3 = 1.5
        self.k_cbf = 1.0

        self.start_tol_pos = 0.3
        self.start_tol_ang = 5*np.pi/180
        self.goal_tol_pos  = 0.5 #0.1
        self.goal_tol_ang  = 5*np.pi/180 #1

        self.cbf_enabled = False

        # ===== [ADD] Acceleration limits =====
        self.a_max = np.array([2.5, 2.5, 3.2])  # [m/s^2, m/s^2, rad/s^2]
        self.a_max = self.a_max * 10 # dt로 인해 qp를 풀었을 때 실제보다 느림. 경험적으로 배수하여 사용
    ...
    def loop(self):
        ...
        # ===== [ADD] Acceleration limits =====
        cons += [
            u[0] - self.u_prev[0] <= self.a_max[0] * self.dt,
            u[0] - self.u_prev[0] >= -self.a_max[0] * self.dt,

            u[1] - self.u_prev[1] <= self.a_max[1] * self.dt,
            u[1] - self.u_prev[1] >= -self.a_max[1] * self.dt,

            u[2] - self.u_prev[2] <= self.a_max[2] * self.dt,
            u[2] - self.u_prev[2] >= -self.a_max[2] * self.dt,
        ]
