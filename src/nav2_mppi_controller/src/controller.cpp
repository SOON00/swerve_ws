...
// #endif
  ...
  // 🌟 누적 시간과 횟수를 기억하는 static 변수
  static int64_t total_duration = 0;
  static int64_t count = 0;
  
  total_duration += duration;
  count++;
  
  // 50번이 너무 길 수 있으므로 10번(약 0.5초)마다 출력하도록 수정
  if (count % 2 == 0) {
    RCLCPP_INFO(logger_, "[MPPI Benchmark] Current: %ld ms | Average: %.2f ms (Samples: %ld)", 
